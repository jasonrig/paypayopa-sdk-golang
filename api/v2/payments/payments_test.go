	...
	}{
		{
			...
			want: &request.Request{
				Url: url.URL{
					...
					Path:   "/v2/payments/fake+payment+id",
				},
				Method:      "DELETE",
...
func TestPost_MakeRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "A post request is constructed with query string",
			fields: fields{
				Environment: api.SandboxEnvironment,
				Params:      testPostParams,
				Payload: PostPayload{
					MerchantPaymentId:   "fake payment id",
					UserAuthorizationId: "fake user auth",
					Amount: api.Amount{
			...
			want: &request.Request{
				Url: url.URL{
					...
					Host:     api.SandboxEnvironment.Host,
					Path:     "/v2/payments",
					RawQuery: "agreeSimilarTransaction=true",
				},
				Method:      "POST",
				ContentType: "application/json",
				Body:        &expectedPayload,
